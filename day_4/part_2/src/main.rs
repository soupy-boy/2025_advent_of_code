...
fn main() {
    ...
    // loop through and remove paper until you can't anymore
    while changed {
        changed = false;
        for row_index in 0..grid.len() {
            for column_index in 0..grid[0].len() {
                ...
                if grid[row_index][column_index] == 1u8 && adjacent_under_four(&grid, row_index as i64, column_index as i64) {
                    removed += 1;
